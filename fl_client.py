...
import numpy as np
import sys
from ml_engine import VASH_MLEngine
from database import get_neo4j_session
from audit_logger import write_worm_log

class VashBankClient(fl.client.NumPyClient):
    def __init__(self, bank_id):
        super().__init__()
        self.bank_id = bank_id
        self.ml_engine = VASH_MLEngine()
...
if __name__ == "__main__":
    ...
    print(f"Starting VASH FL Edge Node for {bank_id}")
    fl.client.start_client(
        server_address=f"127.0.0.1:{port}",
        client=VashBankClient(bank_id).to_client()
    )
