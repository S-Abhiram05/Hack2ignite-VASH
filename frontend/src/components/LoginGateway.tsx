...
export const LoginGateway: React.FC = () => {
  const { setIsAuthenticated, setRole, setComplianceTier, writeAudit, adminAccounts, registerAdminAccount } = useStore();
  ...
  const handleSignupSubmit = async (e: React.FormEvent) => {
    ...
    const randSignature = `VASH-PQC-SIG-${Math.floor(10000000 + Math.random() * 89999999)}-${regHardwareToken.toUpperCase()}`;

    // Update state registry in central mock store
    registerAdminAccount(cleanVpa, regPassword, randSignature, regComplianceLevel);
    localStorage.setItem("vash_user_tier", regComplianceLevel);
    setComplianceTier(regComplianceLevel);
    writeAudit(`Registered new administrative node: VPA '${cleanVpa}' with Hardware ID '${regHardwareToken}' (Tier: ${regComplianceLevel})`, "SUCCESS");
    ...
    const sdkPayload = {
      ...
      issuer: "VASH Dynamic Key Registration Node"
    };
    ...
    setSdkDownloadName(`vash_sdk_${cleanVpa}.json`);
    ...
    <div className="login-gateway-overlay">
      ...
      <div className="login-card">
        <div className="login-header">
          <div className="login-logo">VASH</div>
              ...
              {step === "sdk" && (
                ...
                >
                  ...
                  >
                    ...
                    <div className="drag-desc">Drag & drop your downloaded vash_sdk_[id].json file here</div>
            ...
            {regSuccess ? (
              <div className="sdk-download-box">
                ...
                {sdkDownloadUrl && (
                  ...
                  >
                    Download vash_sdk_{regVpa}.json <Download size={14} />
                  </a>
                )}
              </div>
            ) : (
        ...
        <div style={{ textAlign: "center", marginTop: "16px", fontSize: "11px", color: "var(--color-text-muted)", fontWeight: 600 }}>
          © 2026 Team - VASH. All Rights Reserved. | Developer: Team - VASH
        </div>
      </div>
    </div>
  );
