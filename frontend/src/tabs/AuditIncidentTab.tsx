...
export function generateRfc5424Syslog(log: AuditLog): string {
  ...
  return `<${pri}>1 ${isoTime} vash-soc-node01 audit-daemon 4092 ID47 [meta txId="${log.txId || 'GLOBAL'}" role="${log.role}" status="${log.status}" currHash="${log.currHash.substring(0, 12)}"] VASH_WORM_AUDIT_LOG: ${log.action}`;
}
